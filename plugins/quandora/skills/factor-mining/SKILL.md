...
For each PNG item returned by the manifest whose window is displayable and not withheld, first use `factor_mining_create_backtest_png_download_ticket` when it is visible. Pass the same bare `job_id` used for the manifest and the exact `name` returned by that manifest item. Download the returned short-lived Remote MCP `download_url` directly to `factor_mining_artifacts/<safe_png_name>.png` with local `curl` or a Python HTTP download, verify the saved byte length equals `size_bytes`, and verify `md5_hex` when possible. Do not print, summarize, or persist the `download_url`; it is a temporary Remote MCP URL for the local download only. In `artifact_manifest.json`, record only `saved_path`, `name`, `window`, `size_bytes`, `md5_hex`, and fetch status for the ticket flow.
